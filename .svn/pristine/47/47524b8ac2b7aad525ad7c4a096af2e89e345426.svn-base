package com.teach.android.educationonline.util.system;
...
public class WakeLock {
    ...
    public boolean isScreenOn() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ECLAIR_MR1) {
            MyLog.e("can not call isScreenOn if SDK_INT < 7 ");
            return false;
        } else {
    ...
    public void turnScreenOn() {
        //点亮亮屏
        MyLog.i("PowerManager.WakeLock : wakeLock.isHeld: " + wakeLock.isHeld());
        if (!wakeLock.isHeld()) {
            MyLog.i("PowerManager.WakeLock : 点亮屏幕");
            wakeLock.acquire();
        }
    }
    ...
    public void turnScreenOff() {
        ...
        MyLog.i("PowerManager.WakeLock : wakeLock.isHeld: " + wakeLock.isHeld());
        if (wakeLock.isHeld()) {
            MyLog.i("PowerManager.WakeLock : 灭掉屏幕");
            try {
                wakeLock.release();
            } catch (Exception e) {
                e.printStackTrace();
