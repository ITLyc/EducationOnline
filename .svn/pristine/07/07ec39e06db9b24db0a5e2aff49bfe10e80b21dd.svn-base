...
import com.teach.android.educationonline.log.common.LogConstant;
...
public abstract class Tree implements ITree{
    ...
    private StackTraceElement getCurrentStackTrace() {
        ...
        int stackOffset = getStackOffset(trace, MyLog.class);
        if (stackOffset == -1) {
            return null;
        }
    ...
    private int getStackOffset(StackTraceElement[] trace, Class cla) {
        for (int i = LogConstant.MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (cla.equals(MyLog.class) && i < trace.length - 1 && trace[i + 1].getClassName()
                    .equals(MyLog.class.getName())) {
                continue;
            }
